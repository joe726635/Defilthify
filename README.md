...
When a long operation is expected and the user has no desire to wait, or if it will run late into the night, a $\color{red}{When Done}$ control is available to 'shutdown' or 'sleep' the machine when finished. An abort button is also available to cancel the 'shutdown' within 30 seconds of issuing the command.
